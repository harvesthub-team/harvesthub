...
import { categories, createProduct } from '../services/productManageService';
...
function AddProductPage() {
  ...
  // Product.js unit enum is exactly: kg, g, litre, piece, bunch, dozen.
  const [unit, setUnit] = useState('kg');
  const [stock, setStock] = useState('');
  const [district, setDistrict] = useState(''); // required by Product.js — wasn't collected before
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await createProduct(fields, files);
      // res.data is the real product doc from MongoDB (has a real _id and
      // Cloudinary image URLs) — use that instead of building a fake local
      // object, so the dashboard shows exactly what the backend saved.
      addProduct(res.data);
        ...
        >
          ...
          >
            ...
            <p className="text-white/60 mt-1 text-sm" style={{ fontFamily: 'Inter,sans-serif' }}>
              List your fresh produce to reach buyers across the country.
            </p>
          </div>
